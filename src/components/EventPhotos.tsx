      ...
      <div className="container-custom">
        <div className="text-center max-w-3xl mx-auto mb-12 md:mb-16">
          <div className="inline-flex items-center gap-2 mb-4">
            ...
            <span className="text-gold font-bold uppercase tracking-widest text-xs">Our Gallery</span>
            <div className="h-0.5 w-8 bg-gold" />
          </div>
          <h2 className="text-3xl sm:text-4xl font-heading font-bold text-charcoal dark:text-white mb-6">
            Event <span className="text-deepblue dark:text-gold">Photos</span>
          </h2>
        ...
        <div className="flex flex-col justify-between w-full">
          <div 
            ...
            className="flex flex-row sm:grid sm:grid-cols-2 md:grid-cols-4 overflow-x-auto snap-x snap-mandatory pt-2 pb-3 sm:py-0 px-1 sm:px-0 gap-4 sm:gap-6 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] w-full"
          >
            {eventsData.map((item) => (
              <div 
                key={item.id} 
                className="group relative cursor-pointer overflow-hidden rounded-2xl bg-silver/5 shadow-md hover:shadow-xl transition-all duration-300 border border-border w-full min-w-full sm:min-w-0 sm:w-full shrink-0 snap-center flex flex-col justify-between"
              ...
              >
                <div className="aspect-square relative overflow-hidden shrink-0">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <div className="bg-gold p-3 rounded-full text-charcoal shadow-lg transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                      <Maximize2 className="w-5 h-5" />
                    </div>
                  </div>
                </div>
                <div className="p-4 bg-white dark:bg-charcoal border-t border-border group-hover:border-gold transition-colors flex-grow flex flex-col justify-center">
                  <h3 className="text-sm sm:text-base font-bold text-charcoal dark:text-white text-center truncate">{item.title}</h3>
                  <p className="text-xs text-muted-foreground text-center mt-1">{item.images.length} Photos</p>
                </div>
              </div>
            ))}
          </div>
