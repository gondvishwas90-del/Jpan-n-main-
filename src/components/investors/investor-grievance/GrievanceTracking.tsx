      ...
      <div className="container-custom relative z-10">
        <div className="max-w-4xl mx-auto">
          ...
          <motion.div 
            ...
            className="bg-[#F8FAFC] dark:bg-charcoal/40 border border-[#7BA4D0]/25 p-8 md:p-12 rounded-3xl shadow-xl relative"
          >
             <form onSubmit={handleTrack} className="flex flex-col sm:flex-row gap-4">
                <div className="relative flex-grow">
                   <input 
                    ...
                    className="w-full bg-white dark:bg-charcoal border border-[#7BA4D0]/30 rounded-xl py-4 pl-12 pr-4 text-xs font-bold uppercase tracking-wider focus:outline-none focus:border-[#2E5E99] transition-all shadow-xs text-[#0D2440] dark:text-white placeholder:text-muted-foreground"
                   />
                ...
                <button 
                  type="submit"
                  disabled={isTracking}
                  className="px-8 py-4 bg-[#0D2440] hover:bg-[#2E5E99] text-white font-bold text-xs uppercase tracking-widest rounded-xl transition-all shadow-md flex items-center justify-center gap-3 sm:min-w-[200px]"
                >
                  {isTracking ? (
                    <>
             ...
             {status === 'valid' && (
                <motion.div 
                  ...
                  className="mt-8 p-6 bg-white dark:bg-charcoal border border-[#7BA4D0]/30 rounded-2xl flex items-start gap-4 shadow-sm"
                >
             ...
             {status === 'invalid' && (
                <motion.div 
                  initial={{ opacity: 0, y: 15 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="mt-8 p-6 bg-red-50 dark:bg-red-950/20 border border-red-300 rounded-2xl flex items-start gap-4 shadow-sm"
                >
