...
export interface DriftWallProps<T = any> {
  items?: T[];
...
export function DriftWall<T = DriftWallItem>({
  ...
  const renderTile = (item: any, id: string, colIndex: number) => {
    const isCustom = typeof renderItem === 'function';
    const inner = isCustom ? (
      <div className="drift-wall__inner drift-wall__inner--custom">
    ...
    return (
      <div key={id} tabIndex={0} role="group" aria-label={item.title ?? 'tile'} {...commonProps}>
        {inner}
      </div>
    );
  };
