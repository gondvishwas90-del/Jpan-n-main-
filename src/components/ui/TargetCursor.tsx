...
export const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  const isInsideContainerRef = useRef(!containerRef);
  ...
  useEffect(() => {
    ...
    // If scoped to a container, start invisible until mouse enters container
    if (container) {
      gsap.set(cursor, { opacity: 0 });
      if (hideDefaultCursor) {
        container.classList.add("hide-system-cursor");
      }
    ...
    const moveHandler = (e: MouseEvent) => {
      if (container) {
        ...
        if (isInside && !isInsideContainerRef.current) {
          isInsideContainerRef.current = true;
          gsap.to(cursor, { opacity: 1, duration: 0.2, overwrite: "auto" });
          if (hideDefaultCursor) {
            container.classList.add("hide-system-cursor");
          }
        } else if (!isInside && isInsideContainerRef.current) {
    ...
    const enterHandler = (e: MouseEvent) => {
      ...
      let current: Element | null = directTarget instanceof Element ? directTarget : null;
      ...
      const leaveHandler = () => {
        ...
        if (cornersRef.current) {
          ...
          cornersList.forEach((corner) => gsap.killTweensOf(corner, "x,y"));
    ...
    return () => {
      ...
      if (container) {
        container.classList.remove("hide-system-cursor");
