      ...
      <div className="container-custom">
        ...
        <div className="flex flex-col justify-between w-full">
          <motion.div 
            ...
            className="flex flex-row md:grid md:grid-cols-2 lg:grid-cols-4 overflow-x-auto snap-x snap-mandatory pt-2 pb-4 md:py-2 px-1 md:px-0 gap-5 lg:gap-6 md:overflow-visible [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] w-full"
          >
            {certifications.map((cert, idx) => (
              <motion.div 
                key={idx}
                variants={itemVariants}
                className="group relative bg-[#F8FAFC] dark:bg-charcoal/40 p-7 sm:p-8 rounded-3xl border border-[#7BA4D0]/25 hover:border-[#2E5E99]/50 shadow-xs hover:shadow-[0_20px_50px_-10px_rgba(46,94,153,0.14)] hover:-translate-y-2 transition-all duration-500 w-full min-w-full md:min-w-0 md:w-full shrink-0 snap-center flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-center justify-between mb-6 sm:mb-8">
                    <div className="w-14 h-14 sm:w-16 sm:h-16 bg-[#EBF3FC] dark:bg-white/10 border border-[#7BA4D0]/30 flex items-center justify-center rounded-2xl group-hover:bg-[#0D2440] group-hover:text-white transition-all duration-500 shrink-0 text-[#2E5E99] shadow-xs">
